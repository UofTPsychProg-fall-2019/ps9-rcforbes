...
# load in the data
setwd('/Users/rachelforbes/Documents/GitHub/ps9-rcforbes')
...
ipip.l <- ipip.l %>% 
  separate(expdur,into=c('trait','item'),sep="_")

# Calculate averages for each participant (coded as RID) and trait:
ipip.comp1 <- ipip.l %>% 
  group_by(RID, trait) %>%
  summarise(mtrait=mean(mrt))
...
ipip.comp <- merge(ipip.comp1, ipip.comp2, by = 'RID')
...
age.avg <- ipip.comp %>% 
  group_by(trait)%>%
  mutate(corrcoef = cor(age,mtrait))
